...
export namespace SwapMathY2XDesire {
  ...
  export function y2XRange (
    ...
    if (startHasY) {
      const { acquireX, costY, newLiquidityX } = y2XAtPriceLiquidity(desireX, currentState.sqrtPrice_96, currentState.liquidityX);
    ...
    if (ret.completeLiquidity) {
      ...
      const { acquireX: locAcquireX, costY: locCostY, newLiquidityX } = y2XAtPriceLiquidity(desireX, ret.sqrtLoc_96, currentState.liquidity);
