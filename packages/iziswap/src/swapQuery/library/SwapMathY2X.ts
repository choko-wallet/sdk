...
export namespace SwapMathY2X {
  ...
  export function y2XRange (
    ...
    if (startHasY) {
      const { acquireX, costY, newLiquidityX } = y2XAtPriceLiquidity(
        amountY,
    ...
    if (ret.completeLiquidity) {
      ...
      // trade at locPt
      const { acquireX: locAcquireX, costY: locCostY, newLiquidityX } = y2XAtPriceLiquidity(amountY, ret.sqrtLoc_96, currentState.liquidity);
