...
import type { Address, WalletClient, PublicClient, LocalAccount, TransactionSerializable, Hex,  } from 'viem';
...
export class ContractAccount {
  ...
  public async gaslessExecute(tx: TransactionSerializable[], bundler: string, paymasterUrl: string): Promise<void> {
    const userOp = await this.setAddress()
      .then(builder => builder.setExecution(tx))
      .then(builder => builder.fetchNonce())
      .then(builder => builder.fetchGasPrice())
      .then(builder => builder.setPaymasterAndData(paymasterUrl))
    ...
    const res = await superagent
      ...
      .send({
        ...
        params: [userOp, fixtures.entryPoint],
      })
