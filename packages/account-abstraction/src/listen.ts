...
import type TransactionNotifier from 'messaging-sdk/build/ClientMessenger/TransactionNotifier';
...
export const listenGaslessTxResult = async (
  ...
  return new Promise((resolve, reject) => {
    ...
    clientMessenger.createTransactionNotifier(transactionId, {
      onHashGenerated: (tx: TransactionNotifier) => {
      ...
      onHashChanged: (tx: TransactionNotifier) => {
        // clientMessenger.unsubscribe(transactionId);
      ...
      onMined: (tx: TransactionNotifier) => {
        // clientMessenger.unsubscribe(transactionId);
      ...
      onError: (tx: TransactionNotifier) => {
        // clientMessenger.unsubscribe(transactionId);
        reject(tx.error);
