// Copyright 2021-2022 @choko-wallet/auth-client authors & contributors
// SPDX-License-Identifier: Apache-2.0

export { initEmailProofOfOwnership, validateEmailProofOfOwnership } from './email';

export { initGAProofOfOwnership, validateGAProofOfOwnership } from './ga';

export { validateOAuthProofOfOwnership, preimageOAuthProofOfOwnership } from './oauth';

export { linkUsage, validateUsage } from './usage';
